...
def train_model(
        project_id, model, new_model, task_data, _uuid, template=[], return_num=0,
        **kwargs
):

    _params = dict(hash_id=model.hash_id)
    extra = dict(return_nums=return_num, uuid=_uuid, train_id=new_model.hash_id, version_id=new_model.hash_id)
